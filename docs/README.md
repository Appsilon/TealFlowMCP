...
```
mcp/server/
├── tealflow_mcp.py          # Main entry point (FastMCP server setup)
├── tealflow_mcp/            # Package directory
│   ├── __init__.py          # Package exports
│   ├── core/                # Core configuration
│   │   ├── constants.py     # Constants and paths
│   │   └── enums.py         # Enumerations (ResponseFormat, PackageFilter)
│   ├── data/                # Data loading
│   │   └── loaders.py       # JSON data file loaders with caching
│   ├── models/              # Input validation
│   │   └── input_models.py  # Pydantic models for tool inputs
│   ├── utils/               # Utility functions
│   │   ├── formatters.py    # Response formatting (markdown/JSON)
│   │   └── validators.py    # Module validation and fuzzy matching
│   └── tools/               # MCP tool implementations
│       ├── list_modules.py     # tealflow_list_modules
│       ├── module_details.py   # tealflow_get_module_details
│       ├── code_generation.py  # tealflow_generate_module_code
│       └── other_tools.py      # Search, datasets, templates
└── tests/
...
# Option 2: Using the project's uv
cd /path/to/flow
...
```bash
python mcp/server/tealflow_mcp.py --help
```
...
```json
{
  "mcpServers": {
    "tealflow": {
      "command": "python",
      "args": ["/path/to/flow/mcp/server/tealflow_mcp.py"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "tealflow": {
      "command": "python",
      "args": ["C:\\path\\to\\flow\\mcp\\server\\tealflow_mcp.py"]
    }
  }
}
```
...
```bash
python /path/to/flow/mcp/server/tealflow_mcp.py
```
...
- **Arguments**: `["/absolute/path/to/flow/mcp/server/tealflow_mcp.py"]`
...
The server is located at `mcp/server/tealflow_mcp.py` and references `../../workspace/` for data files.
