...
This module contains the MCP server setup and tool registration.
...
async def list_modules_tool(
    package: str = "all", category: str | None = None, response_format: str = "markdown"
) -> str:
    """
...
async def get_module_details_tool(module_name: str, response_format: str = "markdown") -> str:
    """
    ...
    params = GetModuleDetailsInput(
        module_name=module_name, response_format=ResponseFormat(response_format)
    )
...
async def search_modules_tool(analysis_type: str, response_format: str = "markdown") -> str:
    """
    ...
    params = SearchModulesInput(
        analysis_type=analysis_type, response_format=ResponseFormat(response_format)
    )
...
async def check_dataset_requirements_tool(
    module_name: str, available_datasets: list[str], response_format: str = "markdown"
) -> str:
    """
...
async def list_datasets_tool(response_format: str = "markdown") -> str:
    """
    ...
    Examples:
        ...
        - Get JSON format: response_format="json"
    """
    params = ListDatasetsInput(response_format=ResponseFormat(response_format))
...
async def get_dataset_info_tool(
    file_path: str, include_sample_values: bool = True, response_format: str = "markdown"
) -> str:
    """
...
async def generate_data_loading_tool(
    datasets: list[dict], project_directory: str | None = None, response_format: str = "markdown"
) -> str:
    """
...
async def get_app_template_tool(response_format: str = "markdown") -> str:
    """
    ...
    """
    params = GetAppTemplateInput(response_format=ResponseFormat(response_format))
...
async def generate_module_code_tool(
    module_name: str, parameters: dict | None = None, include_comments: bool = True
) -> str:
    """
    ...
    params = GenerateModuleCodeInput(
        module_name=module_name, parameters=parameters, include_comments=include_comments
    )
...
async def check_shiny_startup_tool(
    app_path: str = ".", app_filename: str = "app.R", timeout_seconds: int = 15
) -> str:
    """
    ...
    params = CheckShinyStartupInput(
        app_path=app_path, app_filename=app_filename, timeout_seconds=timeout_seconds
    )
...
async def setup_renv_environment_tool(
    project_path: str = ".", response_format: str = "json"
) -> str:
    """
    ...
    **Steps performed:**
    ...
    params = SetupRenvEnvironmentInput(
        project_path=project_path, response_format=ResponseFormat(response_format)
    )
...
def run_server() -> None:
    ...
    This function should be called from the package's main() entry point.
    """
    mcp.run()
