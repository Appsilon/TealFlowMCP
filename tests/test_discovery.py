...
class TestFormatHandlers:
    ...
    def test_rds_handler_detection(self):
        """Test RDS format handler detects .Rds files."""
        from tealflow_mcp.tools.handlers import RdsFormatHandler

        handler = RdsFormatHandler()

        assert handler.detect(Path("ADSL.Rds")) is True
        assert handler.detect(Path("ADSL.rds")) is True
        assert handler.detect(Path("ADSL.RDS")) is True
        assert handler.detect(Path("ADSL.csv")) is False

    def test_csv_handler_detection(self):
        """Test CSV format handler detects .csv files."""
        from tealflow_mcp.tools.handlers import CsvFormatHandler

        handler = CsvFormatHandler()

        assert handler.detect(Path("ADSL.csv")) is True
        assert handler.detect(Path("ADSL.CSV")) is True
        assert handler.detect(Path("ADSL.Rds")) is False

    def test_rds_handler_loading_code(self):
        """Test RDS handler generates correct loading code."""
        from tealflow_mcp.tools.handlers import RdsFormatHandler

        handler = RdsFormatHandler()
        code = handler.get_loading_code("ADSL", "data/ADSL.Rds")

        assert code == 'ADSL <- readRDS("data/ADSL.Rds")'

    def test_csv_handler_loading_code(self):
        """Test CSV handler generates correct loading code."""
        from tealflow_mcp.tools.handlers import CsvFormatHandler

        handler = CsvFormatHandler()
        code = handler.get_loading_code("ADTTE", "data/ADTTE.csv")

        assert code == 'ADTTE <- read.csv("data/ADTTE.csv")'

    def test_handler_required_packages(self):
        """Test handlers return correct required packages."""
        from tealflow_mcp.tools.handlers import RdsFormatHandler, CsvFormatHandler

        rds_handler = RdsFormatHandler()
        csv_handler = CsvFormatHandler()

        # Both should require no packages (base R)
        assert rds_handler.get_required_packages() == []
        assert csv_handler.get_required_packages() == []

    def test_get_format_handler(self):
        """Test format handler registry returns correct handler."""
        from tealflow_mcp.tools.handlers import get_format_handler, RdsFormatHandler, CsvFormatHandler

        rds_handler = get_format_handler(Path("ADSL.Rds"))
        assert isinstance(rds_handler, RdsFormatHandler)

        csv_handler = get_format_handler(Path("ADTTE.csv"))
        assert isinstance(csv_handler, CsvFormatHandler)

        unknown_handler = get_format_handler(Path("data.txt"))
